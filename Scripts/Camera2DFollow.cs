...
namespace UnityStandardAssets._2D
{
    public class Camera2DFollow : MonoBehaviour
    {
		...
		private void Update()
        {
			...
			if (newPos.y < startPosition.position.y)
				newPos.y = startPosition.position.y;
			if (newPos.x < startPosition.position.x)
				newPos.x = startPosition.position.x;
			else if (newPos.x > endPosition.position.x)
				newPos.x = endPosition.position.x;
