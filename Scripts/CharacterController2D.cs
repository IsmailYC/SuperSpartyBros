...
public class CharacterController2D : MonoBehaviour {
	...
	void Update()
	{
		...
		if (isGrounded && CrossPlatformInputManager.GetButtonDown ("Jump")) { // If grounded AND jump button pressed, then allow the player to jump
			DoJump ();
		} else if (canDoubleJump && CrossPlatformInputManager.GetButtonDown ("Jump")) //If the player can double jump and the jump bottun is pressed then allow the player to double jump
		{
			DoJump ();
			//set canDoubleJump to false because the player can double jump once
			canDoubleJump= false;
		}
